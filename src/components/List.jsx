...
const List = ({ listOfTodos, onChangeStatus }) => {
  const activeTodosList = listOfTodos.filter(
    (todoItem) => todoItem.active === true
  );
  const inactiveTodosList = listOfTodos.filter(
    (todoItem) => todoItem.active === false
  );
